...
export default function CanvasScrollAnimation({
    ...
    useEffect(() => {
        ...
        for (let i = 1; i <= totalFrames; i++) {
            ...
            img.src = `/${sequenceFolder}/ezgif-frame-${frameNum}.jpg`
    ...
    const isLightBackground = backgroundColor !== "#050505"

    return (
        <div ref={containerRef} className="relative" style={{ height: "450vh", backgroundColor }}>
            {/* Loading */}
            {!imagesLoaded && (
                <div className="fixed inset-0 flex items-center justify-center z-50" style={{ backgroundColor }}>
                    <div className="text-center">
                        <motion.div
                            className="w-16 h-16 border-4 rounded-full mx-auto mb-4"
                            style={{
                                borderColor: isLightBackground ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.2)',
                                borderTopColor: isLightBackground ? '#000' : '#fff'
                            }}
                            animate={{ rotate: 360 }}
                ...
                <div className="absolute inset-0">
                    ...
                    >
                        <div className="text-center px-6">
                            <h1 className="text-4xl md:text-5xl font-bold mb-2" style={{ color: textColor }}>
                                {flavorData.name}
                            </h1>
                            <p className="text-base md:text-lg mb-2" style={{ color: textColor, opacity: 0.7 }}>
                                Premium Ice Cream
                            </p>
                            <div className="text-xl md:text-2xl font-bold" style={{ color: textColor, opacity: 0.9 }}>
                                {flavorData.price}
                            </div>
                        </div>
                    </motion.div>
